...
When("clicks {string}") do |string|                                                                             #Decides button to click based on 'string'
  ...
  elsif string == 'property that changes color'
    @searchResults.clickIdentifiedProperty
  end
...
Then("first property should have {string} under {string} category") do |string, string2|                       #Decides what to verify on Search
  ...
  result_details_window = window_opened_by {@searchResults.clickFirstResult}

  within_window result_details_window do
    if string2 == 'Facilities'
      @resultDetails = SearchDetails.new                                                                       #Initiates page object class 'SearchDetails' instance
      @resultDetails.verifyFacility(string)
    end
  end
...
Then("information in the map pop up and search result should match") do                                         #Verifies information on search result #1 and map pop up for search result #1 matches
  @searchResults.verifyPopUpInfoMatches
end

Then("information of search result {string} and its details should match") do |string|                          #Verifies information on given search result and its result details match
  @resultDetails = SearchDetails.new  
  @resultDetails.verifyDetailsInfoMatches(string)
end